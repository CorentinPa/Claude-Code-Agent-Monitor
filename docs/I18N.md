...
**Supported languages:** English (`en`), Chinese (`zh`), Vietnamese (`vi`), Korean (`ko`), Spanish (`es`), French (`fr`)
...
- Keep key parity across `en`, `zh`, `vi`, `ko`, `es`, and `fr` files for the same namespace.
...
stateDiagram-v2
    ...
    Detecting --> Loaded_fr: localStorage/navigator resolves fr
    ...
    Loaded_en --> Loaded_fr: user switches to fr
    Loaded_fr --> Loaded_en: user switches to en
```
...
|---|---|---|
| Resource parity | Same key coverage across `en/zh/vi/ko/es/fr` | Missing key detection in CI |
...
| Date/number formatting | Locale-specific output shape | `zh-CN`, `vi-VN`, `ko-KR`, `es-ES`, `fr-FR` formatting |
...
| Unexpected fallback to English | Unsupported locale code | Ensure code normalizes to `en|zh|vi|ko|es|fr` and key exists in target namespace |
...
gantt
    ...
    Fill en/zh/vi/ko/es/fr namespace files   :a2, after a1, 5d
...
- [ ] Confirm all namespaces exist for `en`, `zh`, `vi`, `ko`, `es`, `fr`
- [ ] Confirm key parity across all locale JSON files
- [ ] Confirm language switching works in collapsed and expanded sidebar modes
- [ ] Confirm fallback behavior for region tags (e.g., `es-ES`, `vi-VN`, `zh-CN`, `fr-FR`)
