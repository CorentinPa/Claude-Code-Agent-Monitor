...
- Sweep other active sessions whose last activity is older than `DASHBOARD_STALE_MINUTES` (default 180), marking them `abandoned` with their agents `completed`. Healthy Remote Data Source sessions (`source` ≠ `local`) remain mirror-reconciled and excluded; a source in `error`, or stranded in `syncing` for that same window, falls back to this shared stale sweep so old Waiting rows cannot linger forever
