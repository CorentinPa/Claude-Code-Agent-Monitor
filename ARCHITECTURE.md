...
- Extensible -- 14 dual-format Claude/Codex plugins with 66 bundled skills, plus 74 skills discoverable through the skills CLI
...
The verified distribution contains **14 plugins, 66 bundled plugin skills, 18 Claude subagents, 34 Claude commands, 3 CLI helpers, 3 hook configurations, and 2 MCP-enabled plugins**. `npx skills add hoangsonww/Claude-Code-Agent-Monitor --list` discovers **74 total repository skills** because repository-maintenance skills are included alongside plugin skills.
...
- `MCP_DASHBOARD_API_TOKEN`, with `DASHBOARD_API_TOKEN` as a fallback, authenticates to dashboards protected by `DASHBOARD_TOKEN`.
