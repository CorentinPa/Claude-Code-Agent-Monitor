...
| `/sessions`     | Sessions      | `GET /api/sessions?status=&q=&cwd={one-or-more repeated values}&sort_by=&sort_desc=&include_task_progress=true&limit=PAGE_SIZE&offset=page*PAGE_SIZE` — true server-side pagination. Search, project picker, sort, and `status=waiting` remain server-side. The opt-in task flag attaches a bounded `todo_summary` only to visible rows; sessions with observable task state render an accessible micro-donut beside status, with a portal tooltip showing current work, owner labels, source, and up to five tasks. Dashboard/Kanban omit the flag, so their high-volume list calls do not parse transcripts. |
| `/sessions/:id` | SessionDetail | `GET /api/sessions/:id` returns agents, events, workflows, prompt context, and a nullable owner-attributed `todo_snapshot` reduced from Claude Task*/TodoWrite or Codex `update_plan`. The Agents tab places the full task-progress panel immediately after `SessionOverview`, before workflow runs, the agent tree, and cost breakdown. `GET /api/sessions/:id/stats` supplies overview metrics; transcript endpoints supply the cursor-paginated Conversation tab. Waiting and Run-page banners remain above the tabs. |
