...
| `server/lib/codex-home.js` | Resolves the optional `DASHBOARD_CODEX_HOME` (or `CODEX_HOME` / `~/.codex`) and the local `sessions/` / `hooks.json` paths. Settings can validate and persist a dashboard-specific home here; live synchronizers subscribe to its change notifications rather than owning a second scanner. |
