...
| `routes/manual.js`        | Read-only access to the repository's Markdown manual (`GET /api/manual`, `GET /api/manual/:slug`) backing the client's Documentation page, so `docs/*.md` renders inside the dashboard instead of behind a link to a separate site. Nothing is cached or copied: a new document appears on the next request. The slug is matched against the directory listing rather than joined onto a path, so an unknown or traversing slug returns 404 without touching the filesystem. Absent `docs/` yields an empty list and the page's empty state; packaged desktop builds ship the Markdown as an extraResource |
