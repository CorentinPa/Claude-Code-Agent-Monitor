...
| **Remote Data Sources**            | Live remote / multi-machine data collection over SSH. The dashboard mirrors each configured machine's `~/.claude/projects` via **scp** (or `wsl.exe` + `tar` for WSL-hosted Claude on Windows SSH) into a sandboxed per-source staging dir, feeds it through the **same importer** used for local history, and tags the imported sessions with their origin (`sessions.source`). A background poller (`DASHBOARD_REMOTE_SYNC_MS`, default 15 s; `0` disables) keeps it near-real-time — adding or re-enabling a source also triggers an immediate pull — and successful syncs broadcast `remote_data.updated` plus per-session `session_created` / `session_updated` frames so open pages refresh without manual Sync. A global **data-scope** selector (Local only / All / specific sources) narrows the whole app. Sources are managed in **Settings → Remote Data Sources** (add / test / sync / remove) and via `ccam remote-sources`; status streams over `remote_source.status`, and Sessions rows show a source badge for remote-origin sessions. Remote sessions get **no live local hooks**, so healthy sources remain excluded from the local `ps`/`lsof` reap and watchdog transcript scan; each successful sync reconciles their **live status from the mirrored transcript** — the newest JSONL event timestamp (falling back to mirror mtime) within `DASHBOARD_REMOTE_ACTIVE_WINDOW_MS` (default 10 min) means the remote CLI is still running (→ `active`), otherwise it becomes `completed`. If a source reports an SSH `error`, or remains `syncing` longer than `DASHBOARD_STALE_MINUTES`, an `active` session that is itself older than that window falls through to the ordinary stale sweep (`abandoned`, agents completed). A later fresh mirror reactivates a session that is still writing, so unavailable remotes cannot leave permanent Waiting cards. Authentication defers entirely to the host's own SSH stack (`~/.ssh/config`, ssh-agent, keys, `known_hosts`) — no passwords or secrets are stored, external commands run via arg arrays (no shell), and `StrictHostKeyChecking` stays at the SSH default so an unknown host key fails rather than being trusted |
