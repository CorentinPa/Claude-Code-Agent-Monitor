...
| **자동 검색**                     | 세션과 에이전트가 Hook 이벤트로부터 자동으로 생성됩니다. Claude Code는 `SessionStart`에서 즉시 **대기 중** 카드를 만들며, Codex도 Hook 또는 신뢰 승인으로 Hook이 지연될 때 로컬 live-thread 인덱스로 같은 작업을 합니다. 이후 rollout이 같은 행을 보강합니다. |
