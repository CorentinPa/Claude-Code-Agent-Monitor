...
| **MCP 서버(로컬)**                | 세 가지 전송 모드와 16개 도메인 모듈의 97개 타입 지정 도구를 제공하는 종합 로컬 MCP 서버입니다. 범위에는 스코프 데이터, 트랜스크립트/이미지, Claude/GPT 가격, 워크플로, 알림, Webhook, 가져오기/복원, Claude/Codex 구성, Run Agent, 원격 소스, Hook/Home/업데이트, 푸시 및 유지보수가 포함됩니다. 모든 전송이 하나의 검증된 카탈로그를 공유하며 Bearer Token과 계층형 변경/파괴 게이트를 지원합니다 |
...
| **Claude + Codex 플러그인 마켓플레이스** | 하나의 13개 플러그인 소스 트리가 Claude Code와 Codex Manifest, 두 Marketplace Catalog, 62개 번들 스킬, 17개 Claude 서브에이전트, 33개 Claude 명령 및 OpenAI 스킬 메타데이터를 제공합니다. skills.sh CLI는 `npx skills add hoangsonww/Claude-Code-Agent-Monitor --list`로 저장소의 총 70개 스킬을 찾습니다. `claude plugin marketplace add`, `codex plugin marketplace add`, `npx skills add`를 지원합니다 |
...
CCAM은 Claude Code와 Codex가 공유하는 13개 플러그인, 62개 번들 스킬, 17개 Claude 서브에이전트, 33개 Claude 명령, 3개 CLI 도구, 3개 Hook 구성, 2개의 MCP 플러그인을 제공합니다. skills.sh CLI는 저장소 전체에서 70개 스킬을 찾습니다. 전체 카탈로그와 설치 및 검증 절차는 `docs/PLUGINS.md`를 참조하십시오.
...
|-- .claude-plugin/
|   +-- marketplace.json        # Claude Code 마켓플레이스 매니페스트(플러그인 13개)
|-- .agents/plugins/
|   +-- marketplace.json        # Codex 마켓플레이스 매니페스트(플러그인 13개)
|-- plugins/
