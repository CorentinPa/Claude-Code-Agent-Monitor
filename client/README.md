...
`/cc-config` is presented as **Agent Config**. Its Claude Code switch keeps the existing editable, backup-first explorer. Its Codex switch renders `CodexConfigExplorer.tsx`, with a matching overview, stats, scrolling tab rail, redacted previews, and real installed-plugin cards. The explicit editor reads unredacted text only for its server allowlist—`config.toml`, `hooks.json`, user rules, `SKILL.md` files, and Codex/project instructions—so redaction can never destroy real secret values on save. The editor warns that syntax is not validated; saves are atomic and receive a timestamped backup. It subscribes to `codex_config_changed`, so a local CLI, filesystem, or dashboard edit refreshes visible configuration without a page reload.
