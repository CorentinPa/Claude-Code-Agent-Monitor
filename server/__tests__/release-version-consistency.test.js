...
describe("release version consistency", () => {
  ...
  it("keeps every generated plugin manifest on the root release", () => {
    ...
    // sync would leave 28 manifests advertising the previous release with no
    ...
    for (const entry of fs.readdirSync(pluginsDir, { withFileTypes: true })) {
      ...
      for (const format of [".claude-plugin", ".codex-plugin"]) {
        const manifest = path.join("plugins", entry.name, format, "plugin.json");
        if (fs.existsSync(path.join(ROOT, manifest))) manifests.push(manifest);
      }
    }

    assert.ok(manifests.length > 0, "plugin manifests must exist");
    for (const manifest of manifests) {
      assert.equal(
  ...
  it("keeps the marketplace catalog listing every bundled plugin", () => {
    // The catalog is regenerated by the same sync step; a plugin added without
    // re-running it would ship undiscoverable.
    ...
    assert.equal(catalog.plugins.length, dirs.length);
    const listed = new Set(catalog.plugins.map((p) => p.name));
    for (const dir of dirs) {
      assert.ok(listed.has(dir), `${dir} must appear in the marketplace catalog`);
    }
  });
