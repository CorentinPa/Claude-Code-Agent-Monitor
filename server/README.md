...
**Time window (`?from=` and `?to=`).** `GET /api/stats` and `GET /api/pricing/cost` additionally accept an ISO-8601 window, built by `server/lib/range-filter.js`. It composes with the two scopes above; none replaces the others, and a window also forces `/api/stats` onto its dynamic aggregate path (the cached prepared statement is unbounded by construction). Each stats counter is bounded on the timestamp that dates its own rows — `sessions.started_at`, `agents.started_at`, `events.created_at` — so `total_events` agrees with the same `from` sent to `/api/events`; `events_today` and the `*_by_status` maps deliberately ignore the window. `/api/pricing/cost` bounds on `sessions.started_at`, the column it already dates its daily buckets by, because `token_usage` carries no timestamp of its own. **An unparseable bound is ignored, not rejected**: a malformed window must degrade to "no window", never to "no data".
