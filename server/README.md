...
**Task progress.** `server/lib/task-progress.js` stat-caches bounded JSONL reads and reduces the latest observable provider task state without inventing hidden plans. Claude current task mutations (`TaskCreate` / `TaskUpdate` / `TaskList`), legacy `TodoWrite`, task lifecycle events, Codex `update_plan`, and subagent transcripts all normalize into owner-attributed items. `GET /api/sessions?include_task_progress=true` returns a compact `todo_summary` only for the visible durable page; high-volume Dashboard/Kanban calls omit the flag. `GET /api/sessions/:id` returns the full `todo_snapshot`. Missing/malformed transcripts fail soft to `null`.
