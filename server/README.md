...
**Task progress.** `server/lib/task-progress.js` stat-caches JSONL reads, scans only the newest 32 MiB of each transcript at complete-line boundaries, and reduces the latest observable provider task state without inventing hidden plans. Claude current task observations (`TaskCreate` / `TaskGet` / `TaskUpdate` / `TaskList`), legacy `TodoWrite`, task lifecycle events, Codex `update_plan`, and subagent transcripts all normalize into owner-attributed items. `GET /api/sessions?include_task_progress=true` returns a compact `todo_summary` for at most the first 100 returned rows; high-volume Dashboard/Kanban calls omit the flag. `GET /api/sessions/:id` returns the full `todo_snapshot`, capped at 200 task rows. Missing/malformed transcripts fail soft to `null`.
