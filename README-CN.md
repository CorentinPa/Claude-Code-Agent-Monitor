...
| 功能 | 描述 |
|------|------|
| **任务进度** | 根据 Provider 实际暴露的状态按 Agent 归属跟踪任务：当前 Claude 的 `TaskCreate` / `TaskGet` / `TaskUpdate` / `TaskList` 与任务生命周期事件、旧版 `TodoWrite`，以及 Codex `update_plan`。有任务状态的会话会在状态徽标旁显示小型环形进度图，并通过悬停/聚焦 Tooltip 展示预览；会话详情则显示完整进度面板，包括状态分段、当前任务、Agent 归属统计和任务行。Provider 未暴露任务状态时，CCAM 不会虚构进度。 |
