...
| **Tài liệu trong ứng dụng** | Trang **Documentation** tùy chọn tại `/docs` hiển thị sổ tay Markdown của kho mã (`docs/*.md`) ngay trong bảng điều khiển: mục lục, kiểu chữ của ứng dụng, khối Mermaid được vẽ thành sơ đồ, và liên kết giữa các tài liệu chuyển tài liệu tại chỗ. Được phục vụ chỉ đọc bởi `GET /api/manual`; bật mục trong thanh bên ở **Settings → Appearance**. Bản dựng desktop có kèm Markdown nên trang hoạt động ngoại tuyến |
