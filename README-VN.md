...
| **Thị trường plugin Claude + Codex** | Một cây nguồn gồm 13 plugin cung cấp manifest cho Claude Code và Codex, hai catalog marketplace, 62 skill đóng gói, 17 subagent Claude, 33 lệnh Claude và metadata OpenAI. CLI skills.sh phát hiện tổng cộng 70 skill trong repo qua `npx skills add hoangsonww/Claude-Code-Agent-Monitor --list`. Có thể cài bằng `claude plugin marketplace add`, `codex plugin marketplace add` hoặc `npx skills add` |
...
CCAM cung cấp 13 plugin dùng chung cho Claude Code và Codex, với 62 skill đóng gói, 17 subagent Claude, 33 lệnh Claude, 3 công cụ CLI, 3 cấu hình hook và 2 plugin có MCP. CLI skills.sh phát hiện 70 skill trong toàn bộ repo. Xem `docs/PLUGINS.md` để biết catalog, lệnh cài đặt, cơ chế skills.sh và quy trình xác thực.
...
|-- .claude-plugin/
|   +-- marketplace.json        # Manifest marketplace Claude Code (13 plugin)
|-- .agents/plugins/
|   +-- marketplace.json        # Manifest marketplace Codex (13 plugin)
|-- plugins/
